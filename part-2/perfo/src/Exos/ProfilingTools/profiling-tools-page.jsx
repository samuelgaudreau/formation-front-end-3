    ...
    render() {
        ...
        return (
            <div>
                ...
                <button onClick={this.toggleExpensiveRender}>Toggle expensive render, now {useExpensiveRender.toString()}</button>
                {movies.map((x, i) =>  
                    <div key={`${x.title}_${x.year}_${i}`} className="ba ma2">
                        <Movie movie={x} useExpensiveRender={useExpensiveRender} makeRemake={() => this.makeRemake(i)}></Movie>
                    </div>   
                )}
            </div>
        );
